...
def project_images(
    ...
    if in_ipython() and config["images"] is None:
...
def embed_images(
    ...
    if len(image_engine.metadata) > 0:
        ...
        df = df_embeds.merge(df_meta.drop_duplicates(["image_filename"]), on="image_filename")
    ...
    cols_sorted = standard_cols.intersection(df_embeds.columns)
    # append non-standard columns, sorted alphabetically
    cols_sorted = cols_sorted.append(df_embeds.columns.difference(standard_cols).sort_values())
    df = df[cols_sorted]

    if table_format == "csv":
        df_embeds.to_csv(data_dir / f"EmbedImages__{str(uuid.uuid1())}.csv", index=False)
    else:
        df_embeds.to_parquet(data_dir / f"EmbedImages__{str(uuid.uuid1())}.parquet", index=False)
