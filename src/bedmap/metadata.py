...
def get_metadata_list(meta_dir: str) -> list[dict] | list[str]:
    ...
    if meta_dir.endswith(".csv"):
        with open(meta_dir) as f:
            ...
            for i in reader:
                metaList.append({headers[j]: i[j] if len(i) > j and i[j] else "" for j, _ in enumerate(headers)})
...
def get_manifest(imageEngine, atlas_data, **kwargs):
    ...
    manifest = {
        ...
        "default_hotspots": get_hotspots(
            imageEngine, layouts=layouts, n_preproc_dims=kwargs["cluster_preproc_dims"], **kwargs
        ),
