...
function World() {
  this.canvas = document.querySelector('#bedmap-canvas');
...
Lasso.prototype.isLassoEvent = function(e) {
  return e.target.id && e.target.id === 'bedmap-canvas';
}
...
Picker.prototype.onMouseUp = function(e) {
  ...
  if (e.target.id !== 'bedmap-canvas') return; // whether the click hit the gl canvas
...
function Filter(obj) {
  ...
  document.querySelector('#bedmap-canvas').addEventListener('click', this.hide.bind(this));
}
...
function getCanvasSize() {
  var elem = document.querySelector('#bedmap-canvas');
  return {
    w: elem.clientWidth,
    h: elem.clientHeight,
  }
