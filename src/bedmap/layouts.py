...
__all__ = ['cuml_ready', 'cluster_method', 'BaseLayout', 'BaseMetaLayout', 'AlphabeticLayout', 'CategoricalLayout', 'DateLayout',
           'CustomLayout', 'GeographicLayout', 'write_layout', 'get_umap_layout', 'process_single_layout_umap',
           'process_multi_layout_umap', 'save_model', 'load_model', 'get_umap_model', 'get_rasterfairy_layout',
           'get_categorical_boxes', 'get_categorical_points', 'Box', 'process_geojson', 'get_hotspots',
...
## TODO: Profile bedmap on mnist as worst-case for umap (cheap embeddings, high number of points)
...
class BaseLayout(ABC):
    ...
    def get_path(self, filename=None):
        ...
        pathDict = {"out_dir": self.out_dir, "add_hash": self._ADD_HASH, "plot_id": self.plot_id, "gzip": self.gzip}
...
class DateLayout(BaseMetaLayout):
    DEFAULT_OPTIONS = BaseMetaLayout.DEFAULT_OPTIONS.copy()
    DEFAULT_OPTIONS.update({"cols": 3, "bin_units": "years"})
    ...
    def get_layout(self):
        ...
        if os.path.exists(positions_out_path) and os.path.exists(labels_out_path) and self.use_cache:
            return {
        ...
        label_positions = np.array([[grid_x[i * (self.cols + 1)], grid_y[0]] for i in range(len(d))])
...
class GeographicLayout(BaseMetaLayout):
    DEFAULT_OPTIONS = BaseMetaLayout.DEFAULT_OPTIONS.copy()
    DEFAULT_OPTIONS.update({"geojson": None})
...
def process_multi_layout_umap(v, imageEngine, **kwargs):
    ...
    if os.path.exists(model_path) and kwargs["use_cache"] and singleLayout is False:
        ...
        if singleLayout is True:  # Single layout
            ...
            z = model.fit(
                [v for _ in params],
                y=[y if np.any(y) else None for _ in params],
...
def load_model(path: str) -> Union[AlignedUMAP, UMAP]:
    ...
    model.__setattr__(embAtt, list(params.get("umap_attributes").get(embAtt)))
